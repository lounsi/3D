"""
BrainXR - Generateur de cerveau 3D MALADE (Alzheimer)
============================================================
Meme pipeline que cerveau.py, mais force la selection
d'un sujet avec le CDR (Clinical Dementia Rating) le plus eleve,
c'est-a-dire le cerveau le plus atteint par Alzheimer.

Criteres de selection (dans l'ordre) :
  1. CDR le plus eleve (CDR >= 1 = Alzheimer)
  2. MMSE le plus bas (score cognitif le plus faible)

Usage :
    python cerveau_malade.py
"""
...
def find_sickest_subject(oasis):
    """
    Trouve le sujet le plus malade dans le dataset OASIS.
    
    Strategie :
    1. Cherche les sujets avec CDR >= 1 (Alzheimer confirme)
    2. Si aucun CDR >= 1, prend CDR = 0.5 (declin leger)
    3. Parmi les candidats, prend celui avec le MMSE le plus bas
    
    Returns:
        int: index du sujet le plus malade
    """
    ext_vars = oasis.ext_vars
    n = len(ext_vars)
    
    print("\n-> Recherche du cerveau le plus malade...")
    print(f"   {n} sujets disponibles")
    
    # Lister tous les scores CDR
    best_idx = 0
    best_cdr = -1
    best_mmse = 999
    ...
    for i in range(n):
        ...
        candidates.append((i, cdr, mmse))
    
    # Trier : CDR le plus haut d'abord, puis MMSE le plus bas
    candidates.sort(key=lambda x: (-x[1], x[2]))
    
    # Afficher les 5 pires
    print("\n   Top 5 des cerveaux les plus atteints :")
    print(f"   {'Sujet':<10} {'CDR':<8} {'MMSE':<8} {'Statut'}")
    print(f"   {'-'*40}")
    for i, (idx, cdr, mmse) in enumerate(candidates[:5]):
        status = "Alzheimer" if cdr >= 1 else ("Declin" if cdr >= 0.5 else "Sain")
        marker = " <-- SELECTIONNE" if i == 0 else ""
        print(f"   #{idx+1:<9} {cdr:<8.1f} {mmse:<8.0f} {status}{marker}")
    
    chosen_idx = candidates[0][0]
    chosen_cdr = candidates[0][1]
    chosen_mmse = candidates[0][2]
    
    status = "Alzheimer" if chosen_cdr >= 1 else ("Declin leger" if chosen_cdr >= 0.5 else "Sain")
    print(f"\n   Sujet selectionne : #{chosen_idx + 1}")
    print(f"   CDR = {chosen_cdr} ({status})")
    print(f"   MMSE = {chosen_mmse}/30")
    
    return chosen_idx


def generate_sick_brain(oasis, subject_idx):
    """
    Reconstruit le cerveau 3D du sujet malade selectionne.
    Utilise un seuil plus bas pour mieux voir l'atrophie.
    ...
    # --- Parametres optimises pour un cerveau malade ---
    # Seuil plus bas pour capturer plus de details de l'atrophie
    threshold = round(random.uniform(0.15, 0.30), 2)
    sigma = round(random.uniform(0.8, 1.5), 1)
    print(f"\n-> Parametres de reconstruction (optimises cerveau malade) :")
    print(f"   Seuil  : {threshold} (bas pour capturer l'atrophie)")
    ...
    with open(output_file, 'w') as f:
        f.write("# BrainXR - Cerveau 3D MALADE (Alzheimer)\n")
...
def run_ai_analysis(oasis, subject_idx):
    """
    Lance l'analyse IA complete sur le sujet malade.
    Identique a download_real_mri.py.
    """
    ...
    print("\n" + "=" * 60)
    print("    ANALYSE IA DU CERVEAU MALADE")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  BrainXR - Cerveau 3D MALADE (Alzheimer)")
    print("  Ce script selectionne le cerveau le plus atteint !")
    print("=" * 60)

    # Etape 1 : Telecharger OASIS et trouver le plus malade
    from nilearn import datasets
    print("\n-> Telechargement des cerveaux OASIS...")
    oasis = datasets.fetch_oasis_vbm(n_subjects=30)

    subject_idx = find_sickest_subject(oasis)

    # Etape 2 : Generer le cerveau 3D
    obj_file = generate_sick_brain(oasis, subject_idx)

    # Etape 3 : Analyse IA
    report = run_ai_analysis(oasis, subject_idx)

    # Etape 4 : Copier dans Unity
    copy_to_unity(obj_file)

    print(f"\n{'=' * 60}")
    print(f"  PIPELINE CERVEAU MALADE TERMINE !")
    print(f"  ")
    print(f"  Fichiers generes :")
    print(f"    - brain.obj          (modele 3D malade)")
